...
class SecretStoreService {
  ...
  async storeSecret(secret, rawApplication, normalizedApplication, userToken) {
    logger.info(`Storing secret for ${userToken}, application: ${normalizedApplication}, secret:(${secret.length} chars)`)
    ...
    let s3key = `users/${userToken}/data/${normalizedApplication}.json`
